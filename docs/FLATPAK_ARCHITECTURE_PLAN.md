...
| Stdin pairing mode | Add `--pair-token-stdin [--save]`: read one bounded daemon token line from non-interactive piped stdin, validate through the existing authentication controller, optionally persist through the Secret Service wallet, and emit only a bounded token-free JSON result. Add no token flag, discovery path, web surface, manifest permission, or installer behavior. | Tests prove TTY refusal, bounded single-chunk reads, malformed-input rejection, save/validate-only outcomes, fail-closed wallet and controller errors, token-free output, unchanged dispatch modes, and no web/storage surface in the pairing path. |
| Installer auto-pair handoff | After a successful guided companion install, the root installer polls daemon health, then pipes the freshly generated token over stdin only to `--pair-token-stdin --save` run as the original desktop user with their session bus, and launches `--tray` detached on success. The completion screen drops manual token copy/paste only when both steps succeed; every failure falls back to the unchanged manual Web UI instructions. Add no token argv/environment handoff, web injection, Flatpak permission, or sudo/pkexec inside the app. | Installer tests prove the token appears only in the stubbed stdin capture and never in argv, credential-variable scrubbing, session-bus and health gating, nonzero-exit fallback without tray launch, token-free completion output after success, and the retained manual fallback screen. |
