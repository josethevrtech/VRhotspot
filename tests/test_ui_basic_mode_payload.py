...
class TestUiBasicModePayload(unittest.TestCase):
    ...
    def test_basic_mode_ui_labels_and_refresh_contract(self):
        ...
        self.assertIn('id="btnCopySsid">Copy SSID</button>', html)
        ...
        self.assertIn('id="btnShowQrBasic" class="btn icon-only basic-qr-toggle"', html)
        self.assertNotIn('id="btnShowQrBasic">QR Code</button>', html)
        ...
        self.assertIn("const BASIC_PASS_SAVE_RECENT_MS = 3000;", src)
        self.assertIn("async function saveBasicPassphraseIfNeeded", src)
        self.assertIn("if (ev.key !== 'Enter') return;", src)
        self.assertIn("skipRecent: true", src)
        self.assertIn("!cfgDirty && (Date.now() - lastBasicPassphraseSaveAt) < BASIC_PASS_SAVE_RECENT_MS", src)
